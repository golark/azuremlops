...
def clean_data(data):
    ...
    y_df = x_df["y"].apply(lambda s: 1 if s == "yes" else 0)
    
    return x_df, y_df


def perform_training(C, max_iter):
...
def main():
    ...
    run.log("regularization_strength", np.float(args.C))
    run.log("max_iterations", np.int(args.max_iter))

    accuracy = perform_training(args.C, args.max_iter)
